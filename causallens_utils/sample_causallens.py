...
def sample(
    ...
    **model_kwargs,
    ...
    model_kwargs_cd = model_kwargs.copy() # copy model_kwargs for contrastive decoding (CD) only for the first forward pass
    
    # Keep track of generated token history
    generated_tokens_history = []
    
    # Find the starting position of vision tokens
    image_index = torch.nonzero(input_ids == -200)[0, 1].item()
    image_tokens_count = 576  # image token count

    tokenizer = getattr(self, 'tokenizer', None) or model_kwargs.get('tokenizer', None)
    ...
    while True:
        ...
        # Get logits for the next token to be generated
        ...
        generated_tokens = next_tokens.cpu().tolist()
        generated_tokens_history.append(generated_tokens[0])
        

        # Handle Contrastive Decoding (CD) path
        ...
        if use_cd:
            ## CD: forward pass of the model with distorted image input
            ...
            # Use hidden states from the last layer for CD path
            if hasattr(outputs_cd, 'hidden_states') and outputs_cd.hidden_states is not None and len(outputs_cd.hidden_states) > 2:
                target_hidden_state_cd = outputs_cd.hidden_states[-1]
                lm_head = self.get_output_embeddings()
                next_token_logits_cd = lm_head(target_hidden_state_cd[:, -1, :])
            else:
                next_token_logits_cd = outputs_cd.logits[:, -1, :]

            ## CD: pre-process logits from contrastive inputs
            ...
            ## CD: apply temperature warping and top-k filtering in contrastive decoding
